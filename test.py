def display_results(self):
    ...
    candidate_names = {
        ...
        "SEC": {"A": "Cyrus Sinclair", "B": "Meredith Kensington", "C": "Cordelia Harrington",
                "D": "Victoria Ashcroft"},
        "TREAS": {"A": "Lawrence Davenport", "B": "Leopold Thorne", "C": "Atticus Blackwood", "D": "Montgomery Pierce"}
    }
    ...
    print("\n                        Presidential")
    print(
        f"[A]. Derek Hutchins ({pres_votes.get('A', 0)} v) |           [B]. Avery De Mayo ({pres_votes.get('B', 0)} v)")

    print("\n                     Vice Presidential")
    print(
        f"[A]. Aurelia Vance ({vp_votes.get('A', 0)} v)  |           [B]. Evangeline Sinclair ({vp_votes.get('B', 0)} v)")

    print("\n                        Secretarial")
    print(
        f"[A]. Cyrus Sinclair ({sec_votes.get('A', 0)} v) |           [C]. Cordelia Harrington ({sec_votes.get('C', 0)} v)")
    print(
        f"[B]. Meredith K. ({sec_votes.get('B', 0)} v)    |           [D]. Victoria Ashcroft ({sec_votes.get('D', 0)} v)")

    print("\n                         Treasurer")
    print(
        f"[A]. Lawrence D. ({treas_votes.get('A', 0)} v)   |           [C]. Atticus Blackwood ({treas_votes.get('C', 0)} v)")
    print(
        f"[B]. Leopold Thorne ({treas_votes.get('B', 0)} v) |           [D]. Montgomery Pierce ({treas_votes.get('D', 0)} v)")